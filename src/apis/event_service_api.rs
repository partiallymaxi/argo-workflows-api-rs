...
pub async fn event_service_list_workflow_event_bindings(configuration: &configuration::Configuration, namespace: &str, list_options_period_label_selector: Option<&str>, list_options_period_field_selector: Option<&str>, list_options_period_watch: Option<bool>, list_options_period_allow_watch_bookmarks: Option<bool>, list_options_period_resource_version: Option<&str>, list_options_period_resource_version_match: Option<&str>, list_options_period_timeout_seconds: Option<&str>, list_options_period_limit: Option<&str>, list_options_period_continue: Option<&str>, list_options_period_send_initial_events: Option<bool>) -> Result<models::IoPeriodArgoprojPeriodWorkflowPeriodV1alpha1PeriodWorkflowEventBindingList, Error<EventServiceListWorkflowEventBindingsError>> {
    ...
    let uri_str = format!("{}/api/v1/workflow-event-bindings/{namespace}", configuration.base_path, namespace=crate::apis::urlencode(p_namespace));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::IoPeriodArgoprojPeriodWorkflowPeriodV1alpha1PeriodWorkflowEventBindingList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::IoPeriodArgoprojPeriodWorkflowPeriodV1alpha1PeriodWorkflowEventBindingList`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<EventServiceListWorkflowEventBindingsError> = serde_json::from_str(&content).ok();
...
pub async fn event_service_receive_event(configuration: &configuration::Configuration, namespace: &str, discriminator: &str, body: serde_json::Value) -> Result<serde_json::Value, Error<EventServiceReceiveEventError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_namespace = namespace;
    let p_discriminator = discriminator;
    let p_body = body;

    let uri_str = format!("{}/api/v1/events/{namespace}/{discriminator}", configuration.base_path, namespace=crate::apis::urlencode(p_namespace), discriminator=crate::apis::urlencode(p_discriminator));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<EventServiceReceiveEventError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
